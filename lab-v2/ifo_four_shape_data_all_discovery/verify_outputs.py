...
def verify() -> dict:
    audit = pd.read_csv(AUDIT_DIR / "all_input_curves_audit.csv")
    classified = pd.read_csv(FINAL_DIR / "classified_curve_assignments.csv")
    all_status = pd.read_csv(FINAL_DIR / "all_input_curves_final_status.csv")
    ...
    checks = {
        "audit_curve_count_matches_expected": len(audit) == CFG.expected_input_curves,
        "axis_valid_count_matches_expected": int(audit["axis_valid"].sum())
        == CFG.expected_axis_valid_curves,
        "all_status_row_count_matches_expected": len(all_status) == CFG.expected_input_curves,
        "all_numeric_sources_are_under_lab_data_all": audit["source_file"].str.startswith(
            "lab/data_all/"
        ).all(),
        ...
        "minimum_class_size_met": int(counts.min()) >= CFG.minimum_final_class_size,
        "all_four_strict_classes_present": set(strict_counts.index) == set(CLASSES),
        "minimum_strict_class_size_met": int(strict_counts.min())
        ...
        "configured_source_is_exact_lab_data_all": DATA_ROOT.resolve()
        == (ROOT.parents[1] / "lab" / "data_all").resolve(),
