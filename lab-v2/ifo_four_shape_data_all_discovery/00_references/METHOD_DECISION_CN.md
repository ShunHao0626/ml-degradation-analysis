...
唯一输入根目录：`/Users/shunhao/Desktop/ML/lab/data_all`。程序只读取各记录 `accepted/` 下的 CSV；PNG、JSON 与论文 Markdown 不作为数值样本。小时、天、周、月、年统一转换成小时。循环轴、轴单位无法确认、纵轴不能确认为 PCE/效率/功率以及明显损坏的时间尺度均排除；每条记录保留在审计表中。
...
最终选择窗口：500h。前 200h 被定义为快速变化相位，200h 后用于慢衰减判断。每条曲线提取：早期增益、峰后下降、早期下降、谷后恢复及其相对振幅、峰/谷/恢复的时间顺序、早晚斜率与负斜率比例。
...
{
  "rise_gain_abs": 0.006,
  "rise_gain_ratio": 0.15,
  "hill_rapid_drop_ratio": 0.25,
  "valley_drop_ratio": 0.28,
  "valley_recovery_ratio": 0.35,
  "valley_recovery_abs": 0.006,
  "valley_post_recovery_drop_ratio": 0.15,
