#!/usr/bin/env python3
"""Discover Bridge/Hill/Slope/Valley shapes in lab/data_all curves."""
...
os.environ.setdefault("MPLCONFIGDIR", "/tmp/mplconfig_ifo_four_shape_data_all")
os.environ.setdefault("LOKY_MAX_CPU_COUNT", "8")

ROOT = Path(__file__).resolve().parent
REPO = ROOT.parents[1]
DATA_ROOT = REPO / "lab" / "data_all"
...
class Config:
    windows_hours: tuple[int, ...] = (150, 200, 300, 500, 750, 1000, 1500, 2000)
    final_search_windows: tuple[int, ...] = (500, 750, 1000, 1500, 2000)
    ...
    max_plausible_duration_hours: float = 200_000.0
    minimum_unique_points: int = 4
    minimum_final_class_size: int = 10
    expected_input_curves: int = 2_151
    expected_axis_valid_curves: int = 2_134
...
def classify_axis(
    metadata: dict[str, Any], validation_path: Path, csv_path: Path
) -> tuple[bool, float | None, str]:
    _, _, x_text, y_text = axis_text(metadata)
    record_text = " ".join(
        [str(metadata.get("record_id") or ""), validation_path.parent.name]
    ).lower()
    partition_text = " ".join(validation_path.parts).lower()
    if "cycle" in x_text or "x_cycle" in partition_text:
        return False, None, "x_axis_is_cycles_not_time"
    if not (
        re.search(
            r"time|duration|aging|ageing|hour|\bhr\b|\(h\)|\(d\)|day|week|month|year|damp heat|storage|^t\b",
            x_text,
        )
        or re.search(r"x_time_(h|day|min|week)", partition_text)
    ):
        return False, None, "x_axis_or_unit_unverified"
    series_text = csv_path.stem.lower()
    series_label = re.sub(r"__series_\d+$", "", series_text).rsplit("__", 1)[-1]
    explicit_series_pce = bool(
        re.fullmatch(r"pce|efficiency|eff|power|pmax|mppt|spo", series_label)
    )
    explicit_series_non_pce = bool(
        re.fullmatch(r"ff|fill_factor|j_?sc|v_?oc|current|voltage", series_label)
    )
    if explicit_series_non_pce:
        return False, None, "series_is_not_pce_efficiency_or_power"
    if re.search(
        r"voltage|\bvoc\b|current|\bjsc\b|fill factor|\bff\b|humidity|temperature|intensity",
        y_text,
    ):
        return False, None, "y_axis_is_not_pce_efficiency_or_power"
    if not (
        re.search(r"pce|efficien|\beff\b|effecien|power|pmax|mppt|\bspo\b|η", y_text)
        or explicit_series_pce
        or re.search(r"pce|efficien|power|pmax|mppt|\bspo\b", record_text)
    ):
        return False, None, "y_axis_not_pce_efficiency_or_power"
    if re.search(r"year|\byr\b", x_text):
        return True, 8766.0, "years_to_hours"
    if re.search(r"month|\bmon\b", x_text):
        return True, 730.5, "months_to_hours"
    if re.search(r"week|\bwk\b", x_text):
        return True, 168.0, "weeks_to_hours"
    if re.search(r"minute|\bmins?\b", x_text) or "x_time_min" in partition_text:
        return True, 1 / 60.0, "minutes_to_hours"
    if (
        re.search(r"\(d\)|\bd\b|\bdays?\b|dayes", x_text)
        or "x_time_day" in partition_text
    ):
        return True, 24.0, "days_to_hours"
    if re.search(r"\bh\b|\(h\)|hour|\bhr\b", x_text):
        return True, 1.0, "hours"
    if "x_time_h" in partition_text:
        return True, 1.0, "hours_inferred_from_dataset_partition"
...
def make_curve_id(path: Path) -> str:
    ...
    doi = path.parents[2].name
    return f"{doi}_{path.parents[1].name}_{digest}"
...
def audit_dataset(logger: logging.Logger) -> tuple[pd.DataFrame, dict[str, tuple[np.ndarray, np.ndarray]]]:
    ...
    for validation_path in sorted(DATA_ROOT.rglob("validation_result.json")):
        ...
        for csv_path in sorted((validation_path.parent / "accepted").glob("*.csv")):
            axis_ok, factor, axis_status = classify_axis(
                metadata, validation_path, csv_path
            )
...
def evaluate_clustering(data: np.ndarray, labels: np.ndarray) -> dict[str, float]:
    ...
    if len(data) > 500:
        ...
        for label, count in zip(unique, counts):
            ...
            allocation = min(
                len(member_indices), max(2, int(round(500 * count / len(data))))
            )
            sampled.append(rng.choice(member_indices, size=allocation, replace=False))
        sampled_indices = np.concatenate(sampled)
        silhouette_data = data[sampled_indices]
        silhouette_labels = labels[sampled_indices]
    return {
        "silhouette": float(silhouette_score(silhouette_data, silhouette_labels)),
...
def paper_som_search(
    ...
    for index, (stage, window, sigma, learning_rate, iterations) in enumerate(all_grid, start=1):
        ...
        score = (
            ...
            + 0.10
            * min(metrics["min_cluster_size"] / CFG.minimum_final_class_size, 1)
        )
...
def rule_parameter_search(
    ...
    scaled_cache = {
        window: StandardScaler().fit_transform(feature_cache[window][0][FEATURE_COLUMNS])
        for window in CFG.final_search_windows
    }
    evaluation_cache: dict[tuple[int, bytes], dict[str, Any]] = {}
    for candidate_id, values in enumerate(grid, start=1):
        ...
        label_codes = pd.Categorical(labels, categories=CLASSES).codes.astype(np.int8)
        cache_key = (int(window), label_codes.tobytes())
        ...
        if cached is None:
            ...
            metrics = evaluate_clustering(scaled_cache[window], label_codes)
            if minimum_size:
                centroids = np.vstack([shape[labels == label].mean(axis=0) for label in CLASSES])
                correlations = [
                    max_lag_correlation(centroids[i], templates[i], 40) for i in range(4)
                ]
                prototype_score = float(
                    np.mean([(value + 1) / 2 for value in correlations])
                )
            else:
                correlations = [-1.0] * 4
                prototype_score = 0.0
        ...
        minimum_size = cached["minimum_size"]
        metrics = cached["metrics"]
        correlations = cached["correlations"]
        prototype_score = cached["prototype_score"]
        topo_score = cached["topo_score"]
    ...
    logger.info(
        "Shape-rule grid evaluated %d candidates with %d unique assignments",
        len(rows),
    ...
    best_metadata = {
        ...
        "training_data_note": "Only measured CSV curves under lab/data_all are classified; synthetic templates are evaluation targets only and are never added as samples.",
    }
...
def save_final_outputs(
    ...
    all_assignments.to_csv(FINAL_DIR / "all_input_curves_final_status.csv", index=False)
...
def plot_audit(audit: pd.DataFrame) -> None:
    ...
    ax.set(
        xlabel="Available duration (h)",
        ylabel="Number of curves",
        title=f"Duration distribution of {len(valid)} verified curves",
    )
    fig.tight_layout()
...
def validate_and_report(
    ...
    checks = {
        "input_curve_count_matches_expected": len(audit) == CFG.expected_input_curves,
        "all_sources_are_under_lab_data_all": bool(
            audit["source_file"].str.startswith("lab/data_all/").all()
        ),
        "axis_valid_curve_count_matches_expected": int(audit["axis_valid"].sum())
        == CFG.expected_axis_valid_curves,
        ...
        "each_class_meets_minimum_size": min(class_counts.values()) >= CFG.minimum_final_class_size,
        "all_four_strict_topology_classes_exist": set(strict_class_counts) == set(CLASSES),
        "each_strict_class_meets_minimum_size": min(strict_class_counts.values())
...
完整审计了原始 `lab/data_all` 下的 {len(audit)} 条 CSV；其中 {int(audit['axis_valid'].sum())} 条能确认是时间—PCE/效率/功率曲线。时间单位小时、天、周、月、年统一转换成小时。最终模型通过联合评分自动选择 {int(final['window'])}h 时间窗口，共有 {len(classified)} 条真实曲线具备足够时长，没有向训练数据加入任何合成样本。
...
- 所有 {len(audit)} 条曲线均在 `06_final_four_classes/all_input_curves_final_status.csv` 中有最终状态；未分类者保留明确的轴/质量或时长不足原因。
...
唯一输入根目录：`{DATA_ROOT}`。程序只读取各记录 `accepted/` 下的 CSV；PNG、JSON 与论文 Markdown 不作为数值样本。小时、天、周、月、年统一转换成小时。循环轴、轴单位无法确认、纵轴不能确认为 PCE/效率/功率以及明显损坏的时间尺度均排除；每条记录保留在审计表中。
