#!/usr/bin/env python3
"""Merge every dataset whose max time is >= 200 hours into a single folder.
...
curve whose max-x-converted-to-hours is >= 200h, and copies the qualifying
data "as-is" -- including original PNG plots, accepted CSV/PNG series,
validation_result.json, and the paper's `.md` -- into a single destination
folder organised by source axis.
...
Layout produced:
  <BASE>/curves_over_200h_full/
      ...
      x_time_h/<doi_dir>/...
      x_time_min/<doi_dir>/...
      x_time_week_month_year/<doi_dir>/...
...
def merge(base: Path, dest: Path, summary: dict) -> dict:
    """Copy the qualifying figure subfolders (and DOI `.md`) to dest/<top>/<doi>/..."""
    counts = {
        "dois_copied": 0,
        "figures_copied": 0,
        "files_copied": 0,
    ...
    for top, info in summary.items():
        ...
        for doi_name, figures in info["qualifying_dois"].items():
            ...
            doi_dest = top_dest / doi_name
            doi_dest.mkdir(exist_ok=True)
            n_dois += 1
            n_figs += len(figures)

            for fig_name in sorted(figures):
                fig_src = doi_src / fig_name
                fig_dest = doi_dest / fig_name
                if fig_dest.exists():
                    shutil.rmtree(fig_dest)
                shutil.copytree(fig_src, fig_dest)
                # Count files inside for stats
                n_files += sum(1 for _ in fig_dest.rglob("*") if _.is_file())
            ...
            # Also copy the paper markdown (.md) if present at DOI level
            for md in doi_src.glob("*.md"):
                shutil.copy2(md, doi_dest / md.name)
                n_files += 1

        counts["by_top"][top] = {
            "dois_copied": n_dois,
            "figures_copied": n_figs,
            "files_copied": n_files,
        }
        counts["dois_copied"] += n_dois
        counts["figures_copied"] += n_figs
        counts["files_copied"] += n_files
    ...
    with open(dest / "manifest.json", "w", encoding="utf-8") as f:
        json.dump({
            ...
            "totals": {
                "dois": counts["dois_copied"],
                "figures": counts["figures_copied"],
                "files": counts["files_copied"],
            },
...
def main():
    ...
    base = BASE_DIR
    dest = args.dest
    if dest.resolve() == base.resolve() or base in dest.resolve().parents:
        # Sanity check: do not write inside the source base
        pass
    ...
    for top, c in counts["by_top"].items():
        print(f"  {top}: {c['dois_copied']} DOI(s), {c['figures_copied']} figure(s), {c['files_copied']} file(s)")
    print(f"\nTotals -> {counts['dois_copied']} DOI(s), {counts['figures_copied']} figure folder(s), {counts['files_copied']} file(s)")
